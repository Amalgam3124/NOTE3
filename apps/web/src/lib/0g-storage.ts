...
import { Note, ImageUpload } from '@onchain-notes/types';
...
export async function getNote(cid: string): Promise<Note> {
  try {
    ...
    if (note.markdown.startsWith('[SPLIT_NOTE:') && note.markdown.endsWith(']')) {
      console.log('0G Storage: Detected split note, reconstructing...');
      const reconstructedNote = await reconstructSplitNote(note);
  ...
  try {
    ...
    for (let i = 0; i < markdown.length; i++) {
      ...
      if (currentChunk.length >= maxMarkdownSize) {
        ...
        const { putJSON } = await import('@onchain-notes/sdk');
        const { cid } = await putJSON(chunkNote, signer);
 ...
 */
async function reconstructSplitNote(splitNote: Note): Promise<Note> {
  try {
    ...
    for (const chunkCid of chunkCids) {
      try {
        const chunk = await getJSON<any>(chunkCid);
